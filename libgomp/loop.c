...
#endif
      static volatile uint64_t gomp_work_share_id = 1;
#ifdef HAVE_SYNC_BUILTINS
      ws->ws_id = __sync_fetch_and_add (&gomp_work_share_id, 2);
#else
      gomp_mutex_lock (&gomp_parallel_region_id_lock);
      gomp_work_share_id += 2;
      ws->ws_id = gomp_work_share_id;
      gomp_mutex_unlock (&gomp_parallel_region_id_lock);
#endif
      long span = (end - start)/incr;
      gomp_datarace_master_begin_dynamic_work(ws->ws_id, span);
    }
}
