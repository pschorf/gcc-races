...
/* function for datarace detection */
void
gomp_datarace_begin_work(uint64_t region_id, int tid)
{
  return;
}
...
{
  static volatile uint64_t gomp_parallel_region_id = 2;
...
#ifdef HAVE_SYNC_BUILTINS
  team->region_id = __sync_fetch_and_add (&gomp_parallel_region_id, 2);
#else
  gomp_mutex_lock (&gomp_parallel_region_id_lock);
  gomp_parallel_region_id += 2;
		 ...
		 struct gomp_team *team)
{
  ...
  // callback for dispatching master work
  gomp_datarace_begin_work(team->region_id, thr->ts.team_id);
